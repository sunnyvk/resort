...
function myFunction2(x2,mt) {
  if (x2.matches && valnum!==3  ) { 
    setVal(3)
  } 
}
var x2 = window.matchMedia("(min-width: 1000px)")
myFunction2(x2) 
x2.addListener(myFunction2) 

function myFunction3(x3) {
  if (x3.matches && valnum2!==1 ) { 
    setVal2(1)
  } 
  if (!x3.matches && valnum2===1 ) { 
    setVal2(0)
...
var x3 = window.matchMedia("(max-width: 680px)")
myFunction3(x3) 
...
<section className='cottages-slide cottages-slide-2'>
   ...
   <button className='add-cottages-button'> ADD NEW CART</button>
   </div>
     ...
     <div className='dot-parnet' >
      {cottages.map((el,i,arr)=> {
        if((i+1)%(valnum-valnum2)===0 ){
        console.log(i+1)
      return <div className={move===((i+1)/(valnum-valnum2)*100-100)?'dot active-dot':'dot '} onClick={(e)=>{
        setActiveDot(i)
        setMove((val)=>((i+1)/(valnum-valnum2)*100-100))
 ...
 <section className='greatbluesort'>
   <h2 className='greatbluesort-title'>WHAT'S HAPPENING Cuba Goa</h2>
